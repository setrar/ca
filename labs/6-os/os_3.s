...
# timer
_isr4:
  # current running registers -> saved registers current task
  la     sp,kstack            
  lw     t0,0(sp)            
  lw     t1,4(sp)            
  lw     a0,8(sp)           
  ...
  sw     a0,8(sp)              
  sw     a7,12(sp)   
  csrrs t2,uepc,zero   # We read the PC before the exception hapened and save it as the PC of the task
  sw     t2,16(sp)             


  # saved registers previous task -> current running registers
  la     sp,kstack            
  addi sp, sp, 36
  lw     t0,0(sp)             
  lw     t1,4(sp)             
  ...
  sw     a0,8(sp)              
  sw     a7,12(sp) 
  csrrw zero,uepc,t2 # Set the PC after the exception to that of the task that was running previously


  # saved registers current task -> saved registers next task
  la     sp,kstack            
  addi sp, sp, 16
  lw     t0,0(sp)             
  lw     t1,4(sp)             
  ...
  li a0, '*'   # Load character to print in the terminal
	li a7, 11    # Syscall 11 = PrintChar
	ecall        # Print the character
  li a0, 0xffff0018    # Get the current timestamp and save it in t0
  lw t0, (a0)          
  addi t0, t0, 100     # Add 100ms to current timestamp
  li a0, 0xffff0020    # Store the new timeout timestamp into timecmp
  sw t0, (a0)
...
# set the timer to interrupt in 100ms from now
  li a0, 0xffff0018    # Get the current timestamp and save it in t0
  lw t0, (a0)          
  addi t0, t0, 100     # Add 100ms to current timestamp
  li a0, 0xffff0020    # Store the new timeout timestamp into timecmp
...
# globally enable interrupts and timer interrupts
...
# set next running task to be taskB
  la     a0,kstack   
  addi a0, a0, 52   # We want to set up the part of the stack that holds the next sp to use
  la t0, taskB 
  sw     t0,0(a0)             
  

# start with calling taskA
  b taskA
